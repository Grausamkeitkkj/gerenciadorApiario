...
class CadastroProducao:
    ...
    def salvar_dados(self):
        ...
        if caixa == "Selecione a caixa":
            messagebox.showwarning("Aviso", "Por favor, selecione uma caixa")
            return
        ...
        if not quantidade_mel or not data_producao or not caixa:
            ...
            cursor.execute("SELECT id FROM caixas WHERE numero_caixa = %s", (caixa,))
            caixa_id = cursor.fetchone()

            if caixa_id:
                cursor.execute("INSERT INTO producao_mel (quantidade, data_producao, caixa_id) VALUES (%s, %s, %s)", (quantidade_mel_float, data_producao_sql, caixa_id))
                conn.commit()
